...
namespace EgyMartAdminPortal.Services
{
    public class DashboardService(HttpClient httpClient)
    {
        ...
        //29
        public async Task<List<TopSellingProductData>> TopSellingProducts(int periodMonths = 3)
        {
            var response = (await _httpClient.GetFromJsonAsync<ApiResponse<List<TopSellingProductData>>>($"orders/api/Report/TopSellingProducts?periodMonths={periodMonths}"))!;
            return response.Data ?? [];
        }
    }
}
